...
## Recommended Firebase credential format (fixes DECODER unsupported)

Instead of pasting the multiline private key into Netlify, store the complete Firebase service-account JSON file as Base64.

1. Open the downloaded Firebase service-account `.json` file.
2. Convert the complete file to Base64 on Windows PowerShell:

```powershell
[Convert]::ToBase64String([IO.File]::ReadAllBytes("C:\\path\\to\\service-account.json"))
```

3. In Netlify, add a new environment variable:

```text
FIREBASE_SERVICE_ACCOUNT_BASE64=<paste the long Base64 result>
```

4. Keep `FIREBASE_PROJECT_ID`, but you may remove `FIREBASE_PRIVATE_KEY` and `FIREBASE_CLIENT_EMAIL` after the Base64 variable works.
5. Choose **Clear cache and deploy site**.

The functions check `FIREBASE_SERVICE_ACCOUNT_BASE64` first, then fall back to the older variables.
